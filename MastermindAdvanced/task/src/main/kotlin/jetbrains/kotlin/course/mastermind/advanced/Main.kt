...
fun generateSecret(wordLength: Int, alphabet: String): String =
    (1..wordLength).map { alphabet.random() }.joinToString("")
...
fun isCorrectInput(userInput: String, wordLength: Int, alphabet: String): Boolean {
    val isCorrectLength = userInput.length == wordLength
    val isCorrectLetters = userInput.all { it in alphabet }
    return isCorrectLength && isCorrectLetters
}
